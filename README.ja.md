...
| [`/git-branch`](#git-branch) | ブランチ管理: 一覧、切り替え、作成、削除 |
| [`/git-diff`](#git-diff) | AI生成のhunkをインタラクティブにレビューし、承認したものをコミット |
| [`/git-log`](#git-log) | git log を oneline 形式で表示 |
...
AI支援によるhunk分解とインタラクティブな差分レビュー。ファイルツリーとunified diffを左右に表示し、1つの論理的hunkずつ確認・調整・コミットできます。
...
git log を oneline 形式で表示します。ブランチ名と HEAD の位置も表示されます。
