...
- 設定は **ローカル** 設定 (`<リポジトリルート>/pi-git.toml`) に保存されます（Git リポジトリ内の場合）。リポジトリ外の場合は **グローバル** 設定 (`~/.config/pi-git/settings.json`) にフォールバックします。
...
1. **ローカル設定** — `<リポジトリルート>/pi-git.toml`（最優先）
...
| 状況 | 保存先 | 動作 |
|------|--------|------|
| Git リポジトリ内、 `--global` なし | ローカル | `<リポジトリルート>/pi-git.toml` に保存 |
...
| 初回書き込みで両方の設定が存在しない | ローカル（既定値で初期化） | `pi-git.toml` をすべての既定値＋指定値で作成 |
...
**推奨事項:** チームで設定を共有したい場合は `pi-git.toml` をコミットしてください。個人ごとに異なる設定にしたい場合は `pi-git.toml` を `.gitignore` に追加してください。
...
- 設定は `cwd` を考慮して解決されます。monorepo のサブディレクトリから pi を実行しても、リポジトリルートの `pi-git.toml` が正しく読み込まれます。
